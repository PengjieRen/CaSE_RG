...
def train(args):
    ...
    train_dataset = TMemNetDataset(train_samples, None, None, None, None, None, None, None, sample_tensor=torch.load(os.path.join(data_path,  args.dataset+ '.train.TMemNet.dataset.pkl')))

    model = TMemNet(args.embedding_size, args.hidden_size, vocab2id, id2vocab, max_dec_len=args.max_target_length, beam_width=1)
    init_params(model)
...
def test(args):
    ...
    if len(dev_samples) > 10:
        dev_dataset = TMemNetDataset(dev_samples, None, None, None, None, None,None, None, None, sample_tensor=torch.load(os.path.join(data_path, args.dataset+ '.dev.TMemNet.dataset.pkl')))

    test_samples = torch.load(os.path.join(data_path, args.dataset + '.test.pkl'))
    if len(test_samples) > 10:
        test_dataset = TMemNetDataset(test_samples, None, None, None, None, None, None, None, None, sample_tensor=torch.load(os.path.join(data_path, args.dataset + '.test.TMemNet.dataset.pkl')))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--output_path", type=str, default='./output/TMemNet/')
