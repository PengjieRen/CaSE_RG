...
import argparse
from TMemNet.Model import *
from Utils import *
from transformers.optimization import *

base_output_path = './output/TMemNet/'
...
def train(args):
    ...
    train_dataset = TMemNetDataset(train_samples, None, None, None, None, None, None, None, sample_tensor=torch.load(data_path + '.TMemNet.dataset.pkl'))

    model = TMemNet(embedding_size, hidden_size, vocab2id, id2vocab, max_dec_len=max_target_length, beam_width=1)
    init_params(model)
    ...
    for i in range(epoch):
        model_trainer.train_epoch('train', train_dataset, collate_fn, batch_size, i, model_optimizer,
                                  model_scheduler)
...
def test(args):
    ...
    marco_dev_dataset = TMemNetDataset(marco_dev_samples, None, None, None, None, None,None, None, None, sample_tensor=torch.load(data_path + 'marco/marco.dev.TMemNet.dataset.pkl'))

    marco_test_samples = torch.load(data_path + 'marco/marco.test.pkl')
    marco_test_dataset = TMemNetDataset(marco_test_samples, None, None, None, None, None,None, None, None, sample_tensor=torch.load(data_path + 'marco/marco.test.TMemNet.dataset.pkl'))

    cast_test_samples = torch.load(data_path + '.pkl')
    cast_test_dataset = TMemNetDataset(cast_test_samples, None, None, None, None, None, None, None, None, sample_tensor=torch.load(data_path + '.TMemNet.dataset.pkl'))
