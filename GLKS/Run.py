...
import argparse
import os
from GLKS.Model import *
from common.Utils import *
from Utils import *
from transformers.optimization import *

base_output_path = './output/GLKS/'
...
def train(args):
    ...
    train_dataset = GLKSDataset(train_samples, None, None, None, None, None,None, None, None, sample_tensor=torch.load(data_path + '.GLKS.dataset.pkl'))

    model = GLKS(min_window_size, num_windows, embedding_size, hidden_size, vocab2id, id2vocab, max_dec_len=max_target_length, beam_width=1, emb_matrix=None)
    init_params(model)
    ...
    model_trainer = CumulativeTrainer(model, tokenizer, detokenizer, args.local_rank, 4, accumulation_steps=accumulation_steps)

    for i in range(epoch):
        model_trainer.train_epoch('ds_mle_mce_train', train_dataset, collate_fn, batch_size, i, model_optimizer, model_scheduler)
...
def test(args):
    ...
    marco_dev_dataset = GLKSDataset(marco_dev_samples, None, None, None, None, None,None, None, None, sample_tensor=torch.load(data_path + 'marco/marco.dev.GLKS.dataset.pkl'))

    marco_test_samples = torch.load(data_path + 'marco/marco.test.pkl')
    marco_test_dataset = GLKSDataset(marco_test_samples, None, None, None, None, None,None, None, None, sample_tensor=torch.load(data_path + 'marco/marco.test.GLKS.dataset.pkl'))

    cast_test_samples = torch.load(data_path + '.pkl')
    cast_test_dataset = GLKSDataset(cast_test_samples, None, None, None, None, None,None, None, None, sample_tensor=torch.load(data_path + '.GLKS.dataset.pkl'))
    ...
    for i in range(epoch):
        ...
        if os.path.exists(file):
            model = GLKS(min_window_size, num_windows, embedding_size, hidden_size, vocab2id, id2vocab, max_dec_len=max_target_length, beam_width=1)
