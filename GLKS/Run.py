...
def train(args):
    ...
    for i in range(args.epoch):
        model_trainer.train_epoch('ds_mle_mce_train', train_dataset, collate_fn, args.batch_size, i, model_optimizer, model_scheduler)
...
def test(args):
    ...
    if len(dev_samples)>10:
        dev_dataset = GLKSDataset(dev_samples, None, None, None, None, None,None, None, None, sample_tensor=torch.load(os.path.join(data_path, args.dataset+ '.dev.CaSE.dataset.pkl')))

    test_samples = torch.load(os.path.join(data_path, args.dataset + '.test.pkl'))
    if len(test_samples)>10:
        test_dataset = GLKSDataset(test_samples, None, None, None, None, None,None, None, None, sample_tensor=torch.load(os.path.join(data_path, args.dataset+ '.test.CaSE.dataset.pkl')))
    ...
    for i in range(args.epoch):
        ...
        if os.path.exists(file):
            model = GLKS(args.min_window_size, args.num_windows, args.embedding_size, args.hidden_size, vocab2id, id2vocab, max_dec_len=args.max_target_length, beam_width=1)
