...
import argparse
import os
from GTTP.Model import *
from common.Utils import *
from Utils import *
from transformers.optimization import *

base_output_path = './output/GTTP/'
...
def train(args):
    ...
    train_dataset = GTTPDataset(train_samples, None, None, None, None, None, None, None, sample_tensor=torch.load(data_path + '.GTTP.dataset.pkl'))

    model = GTTP(embedding_size, hidden_size, vocab2id, id2vocab, max_dec_len=max_target_length, beam_width=1)
    init_params(model)
...
def test(args):
    ...
    marco_dev_dataset = GTTPDataset(marco_dev_samples, None, None, None, None, None,None, None, sample_tensor=torch.load(data_path + 'marco/marco.dev.GTTP.dataset.pkl'))

    marco_test_samples = torch.load(data_path + 'marco/marco.test.pkl')
    marco_test_dataset = GTTPDataset(marco_test_samples, None, None, None, None, None,None, None, sample_tensor=torch.load(data_path + 'marco/marco.test.GTTP.dataset.pkl'))

    cast_test_samples = torch.load(data_path + '.pkl')
    cast_test_dataset = GTTPDataset(cast_test_samples, None, None, None, None, None, None, None, sample_tensor=torch.load(data_path + '.GTTP.dataset.pkl'))
