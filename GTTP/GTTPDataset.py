...
class GTTPDataset(Dataset):
    def __init__(self, samples, query, passage, vocab2id, num_passage=10, context_len=80, passage_len=200, answer_len=50, n=1E10, sample_tensor=None):
        super(GTTPDataset, self).__init__()
    ...
    def load(self):
        for id in range(len(self.samples)):
            ...
            self.sample_tensor.append([id_tensor, query_tensor, background_tensor, response_tensor, source_map_tensor])
            self.len = id + 1
            if id>=self.n:
                break
...
def collate_fn(data):
    id,context,background,response,source_map = zip(*data)
