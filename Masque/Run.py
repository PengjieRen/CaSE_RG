...
def train(args):
    ...
    train_dataset = MasqueDataset(train_samples, None, None, None, None, None, None, None, None, None, None, sample_tensor=torch.load(os.path.join(data_path,  args.dataset+ '.train.Masque.dataset.pkl')))

    model = Masque(args.max_target_length, id2vocab, vocab2id, args.hidden_size)
    init_params(model)
...
def test(args):
    ...
    if len(dev_samples) > 10:
        dev_dataset = MasqueDataset(dev_samples, None, None, None, None, None, None, None, None, None, None, sample_tensor=torch.load(os.path.join(data_path, args.dataset+ '.dev.Masque.dataset.pkl')))

    test_samples = torch.load(os.path.join(data_path, args.dataset + '.test.pkl'))
    if len(test_samples) > 10:
        test_dataset = MasqueDataset(test_samples, None, None, None, None, None, None, None, None, None, None, sample_tensor=torch.load(os.path.join(data_path, args.dataset + '.test.Masque.dataset.pkl')))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--output_path", type=str, default='./output/Masque/')
